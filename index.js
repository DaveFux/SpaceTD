...
(function () {
  ...
  function load() {
    ...
    tileBackground.load("./assets/map", "mapaAM.json", loaded);
    ...
    spNumeros.load(
      "samples//numeros//numeros.png",
      "samples//numeros//numeros.json",
      loaded
    );
    ...
    spBalas.load(
      "samples//balas//tiros.png",
      "samples//balas//tiros.json",
      loaded
    );
    assets.push(spBalas);
    var spCreepsBlue1 = new SpriteSheet();
    spCreepsBlue1.load(
      "samples//creep//creep-1-blue//sprite.png",
      "samples//creep//creep-1-blue//sprite.json",
      loaded
    );
    assets.push(spCreepsBlue1);
    var spCreepsRed1 = new SpriteSheet();
    spCreepsRed1.load(
      "samples//creep//creep-1-red//sprite.png",
      "samples//creep//creep-1-red//sprite.json",
      loaded
    );
    assets.push(spCreepsRed1);
    var spCreepsGreen1 = new SpriteSheet();
    spCreepsGreen1.load(
      "samples//creep//creep-1-green//sprite.png",
      "samples//creep//creep-1-green//sprite.json",
      loaded
    );
    assets.push(spCreepsGreen1);
    var spCreepsBlue2 = new SpriteSheet();
    spCreepsBlue2.load(
      "samples//creep//creep-2-blue//sprite.png",
      "samples//creep//creep-2-blue//sprite.json",
      loaded
    );
    assets.push(spCreepsBlue2);
    var spCreepsRed2 = new SpriteSheet();
    spCreepsRed2.load(
      "samples//creep//creep-2-red//sprite.png",
      "samples//creep//creep-2-red//sprite.json",
      loaded
    );
    assets.push(spCreepsRed2);
    var spCreepsGreen2 = new SpriteSheet();
    spCreepsGreen2.load(
      "samples//creep//creep-2-green//sprite.png",
      "samples//creep//creep-2-green//sprite.json",
      loaded
    );
    assets.push(spCreepsGreen2);
    var spTorre = new SpriteSheet();
    spTorre.load(
      "samples//tower-defense-turrets//tower-defense-turretsjson.png",
      "samples//tower-defense-turrets//tower-defense-turretsjson.json",
      loaded
    );
    assets.push(spTorre);
  }
  ...
  function mainMenu() {
    ...
    botaoInstrucoes.addEventListener("click", function () {
      ...
      textoInstrucoes.innerHTML =
        "<br/>O jogo chama-se Space Tower Defense e o utilizador pode construir quatro tipos de torres(gelo,chama,canhão e sniper)." +
      ...
      backButton.setAttribute("src", "assets/menu/backButton.png");
  ...
  function setupGame() {
    ...
    playerHealth.setAttribute("id", "outputVida");
    infoVida.appendChild(playerHealth);
    playerHealth.innerHTML = Player.vida;

    shopMenu.appendChild(infoDinheiro);
    shopMenu.appendChild(infoVida);
    ...
    shopMenu.appendChild(btnBase);

    btnBase.addEventListener(
      "click",
      function (e) {
        towerType = "";
        type = "base";
    ...
    // btnTurret1.setAttribute("type", "button");
    btnTurret1.setAttribute(
      "src",
      "samples/tower-defense-turrets/turret-1-1.png"
    );
    shopMenu.appendChild(btnTurret1);

    btnTurret1.addEventListener(
      "click",
      function (e) {
        towerType = "iceTower";
        type = "torre";
    ...
    shopMenu.appendChild(btnTurret2);

    btnTurret2.addEventListener(
      "click",
      function (e) {
    ...
    shopMenu.appendChild(btnTurret6);

    btnTurret6.addEventListener(
      "click",
      function (e) {
    ...
    shopMenu.appendChild(btnTurret7);

    btnTurret7.addEventListener(
      "click",
      function (e) {
    ...
    shopMenu.appendChild(btnUpgrade);
      ...
      function (e) {
        if (radius != undefined) {
          if (radius.upgrade >= 3) {
            ...
            radius.upgradeTorre();
            var up = new Upgrade(
              gSpriteSheets["samples//numeros//numeros.png"],
              0,
              0,
              radius.upgrade
            );
            ...
            up.drawColisionBoundaries(
              canvases.components.ctx,
              true,
              false,
              "white",
              "red"
    ...
    btnBase.appendChild(precoBase);
    btnTurret1.appendChild(precoIceTower);
    btnTurret2.appendChild(precoSniperTower);
    btnTurret6.appendChild(precoCannonTower);
    btnTurret7.appendChild(precoFlameTower);

    var comecarJogo = document.createElement("button");
    comecarJogo.setAttribute("id", "comecarJogo");
    comecarJogo.innerHTML = "Start";
    sideMenu.appendChild(comecarJogo);

    comecarJogo.addEventListener(
      "click",
      function (e) {
    ...
    for (spawn of spawns) {
      var spawn = new refTile(
        gSpriteSheets["samples//casas.png"],
        spawn.x,
        spawn.y,
        spawn.width,
        spawn.height,
  ...
  function criarObjeto(tile, type) {
    switch (type) {
      ...
      case "base":
        ...
        caminhosValidos === caminhosTotais
  ...
  function criarMinion(spawn) {
    var mob = new Minion(
      gSpriteSheets["samples//creep//creep-1-blue//sprite.png"],
      spawn.x,
      spawn.y,
      "normal",
      Game.wave,
  ...
  function colocarTorre(tile, baseB) {
    if (Player.dinheiro - preco >= 0) {
      if (baseB) {
        ...
        var base = new Base(
          gSpriteSheets["samples//base.png"],
          tile.x + tile.width / 4.5,
          tile.y + tile.height / 4.5
        );
        tile.base = base;
        tile.temBase = true;
        ...
        var torre = new Torre(
          ...
          tile.x + tile.width / 7,
          tile.y + tile.height / 7,
          towerType,
          preco
        );
        if (towerType == "iceTower") {
          var aura = new Bala(
            gSpriteSheets["samples//balas//tiros.png"],
            torre.x,
            torre.y,
            torre.type,
            torre.damage,
  ...
  function update() {
        ...
        for (var mob of osMobs) {
          if (torre.type != "iceTower") {
            if (torre.target != undefined) {
              if (
                !(
                  Math.abs(torre.x - torre.target.x) < torre.range * 46 &&
                  Math.abs(torre.y - torre.target.y) < torre.range * 46
                )
              ) {
              ...
              if (
                Math.abs(torre.x - mob.x) < torre.range * 46 &&
                Math.abs(torre.y - mob.y) < torre.range * 46
              ) {
                torre.target = mob;
                torre.rotate();
                if (torre.interval == 0) {
    ...
    for (var i = 0; i < tiles.length; i++) {
      for (var j = 0; j < tiles[i].length; j++) {
        tiles[i][j].drawColisionBoundaries(
          canvases.tiles.ctx,
          true,
          false,
          "black",
        ...
        if (tiles[i][j].hitTestPoint(point.x, point.y)) {
          ...
          if (
            (tiles[i][j].temBase && type == "base") ||
            tiles[i][j].temTorre ||
            Player.dinheiro - preco < 0
          ) {
            tiles[i][j].drawColisionBoundaries(
              canvases.tiles.ctx,
              true,
  ...
  function filtrarMobsAtivos(mob) {
    if (mob.killed == true) Player.dinheiro += mob.drop;

    return (filtrarAtivos(mob))
  }
  ...
  function clearArrays() {
    ...
    osMobs = osMobs.filter(filtrarMobsAtivos);
  ...
  //Render
  function render() {
    canvases.background.ctx.clearRect(
      0,
      0,
      offscreenBackground.width,
      offscreenBackground.height,
      0,
      0,
      offscreenBackground.width,
      offscreenBackground.height
    ...
    canvases.background.ctx.drawImage(
      offscreenBackground,
      0,
      0,
      offscreenBackground.width,
